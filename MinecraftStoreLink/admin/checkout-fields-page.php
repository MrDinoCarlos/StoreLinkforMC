<?php
// admin/checkout-fields-page.php
...
function minecraftstorelink_checkout_fields_page() {
    if (isset($_POST['minecraftstorelink_checkout_fields_nonce']) && wp_verify_nonce($_POST['minecraftstorelink_checkout_fields_nonce'], 'minecraftstorelink_save_checkout_fields')) {
        $fields = isset($_POST['checkout_fields']) ? (array) $_POST['checkout_fields'] : [];
    ...
    ?>
    <div class="wrap">
        <h1>Checkout Field Settings</h1>
        <form method="post">
            <?php wp_nonce_field('minecraftstorelink_save_checkout_fields', 'minecraftstorelink_checkout_fields_nonce'); ?>

            <table class="form-table">
                <tr><th>Fields to ask during checkout:</th></tr>
                <?php foreach ($all_fields as $key => $label): ?>
                    <tr>
                        <th scope="row"><?php echo esc_html($label); ?></th>
                        <td><input type="checkbox" name="checkout_fields[]" value="<?php echo esc_attr($key); ?>" <?php checked(in_array($key, $selected_fields)); ?>></td>
                    </tr>
                <?php endforeach; ?>
            </table>

            <p class="description">Only the selected fields will be shown during WooCommerce checkout.</p>
...
add_filter('woocommerce_checkout_fields', function ($fields) {
    ...
    foreach ($fields['billing'] as $key => $value) {
        if (strpos($key, 'billing_') === 0 && !in_array($key, $allowed)) {
...
add_filter('woocommerce_checkout_fields', function ($fields) {
    ...
    $fields['billing']['minecraft_username'] = [
        ...
        'description' => $mc_name ? __('This field is auto-filled from your linked Minecraft account.', 'minecraftstorelink') : __('Link your Minecraft account to auto-fill this.', 'minecraftstorelink'),
        'priority' => 5,
    ];
...
add_action('woocommerce_checkout_update_order_meta', function ($order_id) {
    if (isset($_POST['minecraft_username'])) {
        update_post_meta($order_id, '_minecraft_username', sanitize_text_field($_POST['minecraft_username']));
    }
});
